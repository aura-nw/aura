...
WHERE fee_grant_allowance.height <= excluded.height`
	...
	_, err = db.Sqlx.Exec(stmt, allowance.Grantee, allowance.Granter, allowanceJSON, allowance.Height)
	if err != nil {
...
func (db *Db) DeleteFeeGrantAllowance(allowance types.GrantRemoval) error {
	...
	_, err := db.Sqlx.Exec(stmt, allowance.Grantee, allowance.Granter, allowance.Height)

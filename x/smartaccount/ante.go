...
func handleSmartAccountTx(
	...
	if simulate {
		return sdkerrors.Wrap(types.ErrNotSupported, "Simulation of SmartAccount txs isn't supported yet")
	}
...
func handleSmartAccountActivate(
	...
	if !signer.Equals(predicAddr) {
		return sdkerrors.Wrap(types.ErrInvalidAddress, "not the same as predicted")
	}
	...
	if !ctx.IsCheckTx() && !ctx.IsReCheckTx() && !simulate {
		...
		if !isBase && !isSa {
			return sdkerrors.Wrap(types.ErrAccountNotFoundForAddress, signer.String())
		}

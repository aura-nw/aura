...
const (
	...
	FlagAllowedContract = "allowed-contract"
)
...
%s tx %s grant cosmos1skjw... cosmos1skjw... --spend-limit 100stake --expiration 2022-01-30T15:04:05Z 
	--allowed-messages "/cosmos.gov.v1beta1.MsgSubmitProposal,/cosmos.gov.v1beta1.MsgVote"
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			allowedContract, err := cmd.Flags().GetStringSlice(FlagAllowedContract)
			if err != nil {
				return err
			}

			if len(allowedContract) > 0 {
				grant, err = customfeegrant.NewAllowedContractAllowance(grant, allowedContract)
				if err != nil {
					return err
				}
			}
	...
	cmd.Flags().StringSlice(FlagAllowedContract, []string{}, "Set of allowed contract address for fee allowance")

...
VALUES `
	...
	_, err := db.Sqlx.Exec(selfDelegationAccQuery, selfDelegationParam...)
	if err != nil {
	...
	_, err = db.Sqlx.Exec(validatorQuery, validatorParams...)
	if err != nil {
...
WHERE validator_info.height <= excluded.height`
	_, err = db.Sqlx.Exec(validatorInfoQuery, validatorInfoParams...)
	if err != nil {
...
WHERE validator_commission.height <= excluded.height`
	_, err = db.Sqlx.Exec(stmt, consAddr.String(), commission, minSelfDelegation, data.Height)
	if err != nil {
...
func (db *Db) SaveValidatorsStatuses(statuses []types.ValidatorStatus) error {
	...
	_, err := db.Sqlx.Exec(validatorStmt, valParams...)
	if err != nil {
...
WHERE validator_status.height <= excluded.height`
	_, err = db.Sqlx.Exec(statusStmt, statusParams...)
	if err != nil {
...
VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`
	_, err = db.Sqlx.Exec(stmt, evidence.Height, voteA, voteB)
	if err != nil {

...
func TestPubkeyToAny(t *testing.T) {
	for _, tc := range []struct {
		...
		_, err := auranw.PubKeyToAny(app.MakeEncodingConfig().Marshaler, []byte(tc.raw))
		if tc.err {
			require.Error(t, err)
		} else {
...
func TestPubKeyDecode(t *testing.T) {
	...
	_, err = auranw.PubKeyDecode(pubKey)
	require.Error(t, err)

	raw := "{\"@type\":\"/cosmos.crypto.secp256k1.PubKey\",\"key\":\"A/2t0ru/iZ4HoiX0DkTuMy9rC2mMeXmiN6luM3pa+IvT\"}"
	any, err := auranw.PubKeyToAny(app.MakeEncodingConfig().Marshaler, []byte(raw))
	require.NoError(t, err)
	dPubKey, err := auranw.PubKeyDecode(any)
	require.NoError(t, err)

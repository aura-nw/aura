...
func (db *Db) pruneBank(height int64) error {
	_, err := db.Sqlx.Exec(`DELETE FROM supply WHERE height = $1`, height)
	if err != nil {
...
func (db *Db) pruneStaking(height int64) error {
	_, err := db.Sqlx.Exec(`DELETE FROM staking_pool WHERE height = $1`, height)
	if err != nil {
		return fmt.Errorf("error while pruning staking pool: %s", err)
	}

	_, err = db.Sqlx.Exec(`DELETE FROM validator_commission WHERE height = $1`, height)
	if err != nil {
		return fmt.Errorf("error while pruning validator commission: %s", err)
	}

	_, err = db.Sqlx.Exec(`DELETE FROM validator_voting_power WHERE height = $1`, height)
	if err != nil {
		return fmt.Errorf("error while pruning validator voting power: %s", err)
	}

	_, err = db.Sqlx.Exec(`DELETE FROM validator_status WHERE height = $1`, height)
	if err != nil {
		return fmt.Errorf("error while pruning validator status: %s", err)
	}

	_, err = db.Sqlx.Exec(`DELETE FROM double_sign_vote WHERE height = $1`, height)
	if err != nil {
		return fmt.Errorf("error while pruning double sign votes: %s", err)
	}

	_, err = db.Sqlx.Exec(`DELETE FROM double_sign_evidence WHERE height = $1`, height)
	if err != nil {
...
func (db *Db) pruneMint(height int64) error {
	_, err := db.Sqlx.Exec(`DELETE FROM inflation WHERE height = $1`, height)
	return fmt.Errorf("error while pruning inflation: %s", err)
}

func (db *Db) pruneDistribution(height int64) error {
	_, err := db.Sqlx.Exec(`DELETE FROM community_pool WHERE height = $1`, height)
	if err != nil {
...
func (db *Db) pruneSlashing(height int64) error {
	_, err := db.Sqlx.Exec(`DELETE FROM validator_signing_info WHERE height = $1`, height)
	if err != nil {
		return fmt.Errorf("error while pruning validator signing info: %s", err)
	}

	_, err = db.Sqlx.Exec(`DELETE FROM slashing_params WHERE height = $1`, height)
	if err != nil {
